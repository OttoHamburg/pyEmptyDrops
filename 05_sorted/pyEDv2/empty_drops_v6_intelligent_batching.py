#!/usr/bin/env python3
"""
Intelligent Batching EmptyDrops implementation (v6).

This version implements gradient-based intelligent batching:
- Conservative batching for high-count cells (steep gradients)
- Aggressive batching for low-count cells (flat gradients)
- Adaptive thresholds based on position in ranked list
"""
...
# --- Intelligent Batching Functions ---
...
@njit
def _log_multinomial_prob_intelligent(indices, data, total_count, log_ambient, log_factorial_total):
    """Intelligent log-probability calculation."""
...
def _create_intelligent_batches(unique_totals, total_lengths, max_batches=200):
    """
    Create intelligent batches based on gradient analysis and position.
    
    Strategy:
    1. High-count cells (first ~200): Conservative 1-2% threshold
    2. Mid-range cells: Moderate 2-5% threshold  
    3. Low-count cells: Aggressive 5-10% threshold
    4. Use gradient to detect steep vs flat regions
    """
    if len(unique_totals) <= max_batches:
        return unique_totals, total_lengths, np.arange(len(unique_totals))
    
    # Sort by total count (descending, like R's ranking)
    sort_idx = np.argsort(-unique_totals)  # Descending order
    sorted_totals = unique_totals[sort_idx]
    sorted_lengths = total_lengths[sort_idx]
    
    # Calculate gradients to identify steep vs flat regions
    gradients = np.abs(np.gradient(sorted_totals))
    ...
    while i < len(sorted_totals) and len(batched_totals) < max_batches:
        ...
        current_gradient = gradients[i] if i < len(gradients) else 0
        
        # Determine batching strategy based on position and gradient
        position_ratio = i / len(sorted_totals)
        
        if position_ratio < 0.1:  # First 10% - high count cells
            # Conservative batching for high-count cells
            pct_threshold = 0.02  # 2%
            max_batch_size = 5
        elif position_ratio < 0.5:  # Next 40% - mid-range cells
            # Moderate batching
            pct_threshold = 0.05  # 5%
            max_batch_size = 20
        else:  # Last 50% - low-count cells
            # Aggressive batching for low-count cells
            pct_threshold = 0.10  # 10%
            max_batch_size = 50
        
        # Adjust threshold based on gradient (steeper = more conservative)
        if current_gradient > np.percentile(gradients, 75):
            pct_threshold *= 0.5  # More conservative for steep regions
            max_batch_size = max(1, max_batch_size // 2)
        elif current_gradient < np.percentile(gradients, 25):
            pct_threshold *= 2.0  # More aggressive for flat regions
            max_batch_size *= 2
        
        # Create batch starting from current position
        batch_totals = [current_total]
        batch_lengths = [sorted_lengths[i]]
        
        j = i + 1
        batch_size = 1
        while (j < len(sorted_totals) and 
               batch_size < max_batch_size and
               len(batched_totals) < max_batches):
            
            next_total = sorted_totals[j]
            
            # Calculate percentage difference
            if current_total > 0:
                pct_diff = abs(next_total - current_total) / current_total
            else:
                pct_diff = 1.0  # Force break if current_total is 0
        ...
        # Use median of batch as representative
    ...
    print(f"    Intelligent batching summary:")
    print(f"      High-count region batches: {sum(1 for i, total in enumerate(batched_totals) if i < len(batched_totals) * 0.1)}")
    print(f"      Mid-range region batches: {sum(1 for i, total in enumerate(batched_totals) if 0.1 <= i/len(batched_totals) < 0.5)}")
    print(f"      Low-count region batches: {sum(1 for i, total in enumerate(batched_totals) if i/len(batched_totals) >= 0.5)}")
    
    return np.array(batched_totals), np.array(batched_lengths), batch_mapping

@njit(parallel=True)
def _monte_carlo_intelligent_batched(
    ...
    """
    Intelligent batched Monte Carlo simulation.
    
    Uses the intelligent batching strategy to minimize multinomial calls
    while maintaining accuracy through gradient-aware grouping.
    """
...
def good_turing_ambient_pool_intelligent(data_csr, low_count_mask):
    """Intelligent ambient profile estimation."""
...
def barcode_ranks_intelligent(data_csr, lower=100, exclude_from=50):
    """Intelligent barcode ranks calculation."""
...
# --- Main Intelligent Batching EmptyDrops Function ---

def empty_drops_v6_intelligent(
    data: sc.AnnData,
    lower: int = 100,
    niters: int = 10000,
    ...
    Uses gradient-based intelligent batching for optimal performance/accuracy trade-off.
    """
    start_time = time.time()
    print("--- Starting EmptyDrops v6 (Intelligent Batching) ---")
    ...
    print(f"  Found {np.sum(test_mask)} cells to test (>{lower} UMIs)")
    print(f"  Found {np.sum(~test_mask)} ambient cells (≤{lower} UMIs)")
    
    # === STEP 2: AMBIENT PROFILE ===
    print("Step 2: Computing ambient profile...")
    low_count_mask = totals <= lower
    ambient_props = good_turing_ambient_pool_intelligent(data_csr, low_count_mask)
    ...
    for i in tqdm(range(n_test_cells), desc="Intelligent log-probs"):
        ...
        if start_idx < end_idx:
            ...
            obs_log_probs[i] = _log_multinomial_prob_intelligent(
                indices, data_vals, total_count, log_ambient, log_factorial_total
            )
        else:
            obs_log_probs[i] = -np.inf
    
    # === STEP 4: INTELLIGENT BATCHED MONTE CARLO ===
    print(f"Step 4: Running {niters} Monte Carlo simulations (intelligent batching)...")
    ...
    batched_totals, batched_lengths, batch_mapping = _create_intelligent_batches(
    ...
    print(f"  Intelligent batches: {len(batched_totals)}")
    ...
    n_above_ordered = _monte_carlo_intelligent_batched(
        batched_totals, batched_lengths, batch_mapping,
        unique_totals, total_lengths, ordered_probs, 
        ambient_props, log_ambient, niters, seed_base
    )
    ...
    if retain is None:
        try:
            retain = barcode_ranks_intelligent(data_csr, lower=lower)
    ...
    print(f"--- EmptyDrops v6 finished in {total_runtime:.2f} seconds ---")
    ...
    if return_metadata:
        metadata = {
            ...
            'version': 'v6_intelligent_batching',
