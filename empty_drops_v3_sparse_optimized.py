#!/usr/bin/env python3
"""
Sparse-optimized EmptyDrops implementation (v3).

This version focuses on maximum sparse matrix efficiency, similar to R's dgCMatrix approach.
Key optimizations:
- Pre-convert to CSR format once
- Remove cell/gene names during computation
- Use pure sparse operations throughout
- Optimize Monte Carlo for sparse data
"""
...
# --- Sparse-Optimized Helper Functions ---
...
@njit
def _log_multinomial_prob_sparse_optimized(indices, data, total_count, ambient_props):
    """
    Ultra-fast sparse log-probability calculation.
    
    Parameters
    ----------
    indices : np.ndarray
        Non-zero gene indices for this cell
    data : np.ndarray  
        Non-zero counts for this cell
    total_count : int
        Total UMI count for this cell
    ambient_props : np.ndarray
        Ambient proportions for all genes
    """
    ...
    for i in range(len(indices)):
        gene_idx = indices[i]
        count = data[i]
        if ambient_props[gene_idx] > 0:
            log_prob += count * np.log(ambient_props[gene_idx]) - _gammaln_numba(count + 1)
        elif count > 0:
            return -np.inf
...
@njit
def _monte_carlo_sparse_optimized(unique_totals, total_lengths, ordered_probs, ambient_props, niters, seed):
    """
    Sparse-optimized Monte Carlo simulation.
    
    This version is optimized for sparse data by:
    - Pre-computing log values
    - Minimizing operations per iteration
    - Using efficient sparse sampling
    """
    ...
    # Process each unique total count
...
def good_turing_ambient_pool_sparse(data_csr, low_count_mask):
    """
    Sparse-optimized ambient profile estimation.
    
    Parameters
    ----------
    data_csr : scipy.sparse.csr_matrix
        Sparse count matrix in CSR format
    low_count_mask : np.ndarray
        Boolean mask for low-count cells
    """
    # Sum across low-count cells using sparse operations
...
def _find_curve_bounds_r_style(x, y, exclude_from=50):
    """R's .find_curve_bounds function exactly."""
...
def barcode_ranks_sparse(data_csr, lower=100, exclude_from=50):
    """
    Sparse-optimized barcode ranks calculation.
    ...
    lower : int
        Lower threshold
    exclude_from : int
        Exclusion parameter for curve fitting
    """
    # Fast sparse sum operation
    totals = np.asarray(data_csr.sum(axis=1)).flatten().astype(int)
    
    # R-style ranking and curve fitting (same as before)
    o = np.argsort(-totals)
...
# --- Main Sparse-Optimized EmptyDrops Function ---

def empty_drops_v3_sparse(
    data: sc.AnnData,
    lower: int = 100,
    niters: int = 10000,
    ...
    This version maximizes sparse matrix efficiency for dramatic speed improvements.
    """
    start_time = time.time()
    print("--- Starting EmptyDrops v3 (Sparse-Optimized) ---")
    
    original_data = data.copy()
    
    # === STEP 1: SPARSE MATRIX OPTIMIZATION ===
    print("Step 1: Optimizing sparse matrix format...")
    
    # Filter genes once
    sc.pp.filter_genes(data, min_counts=1)
    
    # Convert to CSR format once and reuse (this is the key optimization!)
    data_csr = data.X.tocsr()
    print(f"  Matrix format: {data_csr.format}")
    print(f"  Sparsity: {(1 - data_csr.nnz / (data_csr.shape[0] * data_csr.shape[1])) * 100:.4f}%")
    
    # Fast sparse operations for totals
    totals = np.asarray(data_csr.sum(axis=1)).flatten()
    test_mask = totals > lower
    
    print(f"  Found {np.sum(test_mask)} cells to test")
    
    # === STEP 2: SPARSE AMBIENT PROFILE ===
    print("Step 2: Computing ambient profile (sparse)...")
    low_count_mask = totals <= lower
    ambient_props = good_turing_ambient_pool_sparse(data_csr, low_count_mask)
    
    # === STEP 3: SPARSE LOG-PROBABILITY CALCULATION ===
    print("Step 3: Calculating observed log-probabilities (sparse-optimized)...")
    
    # Extract test data as sparse matrix
    test_data_csr = data_csr[test_mask, :]
    n_test_cells = test_data_csr.shape[0]
    obs_log_probs = np.zeros(n_test_cells)
    
    # Ultra-fast sparse log-probability calculation
    for i in tqdm(range(n_test_cells), desc="Sparse log-probs"):
        start_idx = test_data_csr.indptr[i]
        end_idx = test_data_csr.indptr[i + 1]
        
        if start_idx < end_idx:  # Cell has non-zero counts
            indices = test_data_csr.indices[start_idx:end_idx]
            data_vals = test_data_csr.data[start_idx:end_idx]
            total_count = int(data_vals.sum())
            
            obs_log_probs[i] = _log_multinomial_prob_sparse_optimized(
                indices, data_vals, total_count, ambient_props
            )
        else:
            obs_log_probs[i] = -np.inf  # Empty cell
    
    # === STEP 4: SPARSE MONTE CARLO SIMULATION ===
    print(f"Step 4: Running {niters} Monte Carlo simulations (sparse-optimized)...")
    
    test_totals = np.asarray(test_data_csr.sum(axis=1)).flatten().astype(int)
    
    # R-style ordering and grouping
    ...
    print(f"  Processing {len(unique_totals)} unique total counts...")
    print(f"  Total counts range: {unique_totals.min()} to {unique_totals.max()}")
    
    start_mc_time = time.time()
    n_above_ordered = _monte_carlo_sparse_optimized(
    ...
    if retain is None:
        try:
            # Use sparse-optimized knee detection
            retain = barcode_ranks_sparse(data_csr, lower=lower)
    ...
    print(f"--- EmptyDrops v3 finished in {total_runtime:.2f} seconds ---")
